...
"""
ActorCriticElevationNetMode12L: R(2+1)D处理特权历史高程图 + MLP处理本体观测
...
class ActorCriticElevationNetMode12L(nn.Module):
    """Mode12L: R(2+1)D处理特权历史高程图 + MLP处理本体观测
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode12L 网络结构")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode12L_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode12L策略导出为ONNX格式
        ...
        # 创建ElevationNet Mode12L专用的导出器
        exporter = _ElevationNetMode12LOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode12LOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode12L策略的ONNX导出器"""

    def __init__(self, policy: ActorCriticElevationNetMode12L, normalizer=None, verbose=False):
        super().__init__()
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - Mode12L):")
        print(f"{'='*80}")
