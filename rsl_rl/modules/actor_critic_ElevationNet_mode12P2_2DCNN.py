...
"""
ActorCriticElevationNetMode12P2_2DCNN: VAE架构的编码器-解码器网络 (2D CNN版本)

网络结构（修正版）:

    Critic Pipeline (PPO):
        本体观测值（单帧，例如G1中107维）-> MLP -> 特征向量
        高程图历史值（5帧，例如G1中5×425=2125）-> 2D CNN -> 特征向量
        [两个特征concat] -> MLP Critic -> V
    
    Encoder-Decoder Pipeline (VAE架构):
        高程图编码器 (VAE):
            高程图历史值（critic的高程图，5帧无噪声）-> 2D CNN -> 
...
与Mode12P2的主要区别:
    - 使用2D CNN替代R(2+1)D CNN处理高程图
    - 将时间维度作为通道维度处理，使用标准2D卷积
"""
...
class Elevation2DCNNEncoder(nn.Module):
    ...
    将时间维度作为通道维度，使用标准2D卷积处理
    
    输入: [B, T, H, W] -> reshape为 [B, T, H, W]
    处理: 使用2D CNN，将T作为通道维度
    """
    ...
    def _make_2d_block(self, in_channels, out_channels, kernel_size, stride):
        """创建一个2D CNN块"""
        # 2D空间卷积
        conv = nn.Sequential(
            nn.Conv2d(
...
class ActorCriticElevationNetMode12P2_2DCNN(nn.Module):
    """
    完整的Actor-Critic网络，包含VAE编码器-解码器 (2D CNN版本)
    """
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # Critic的高程图编码器（处理高程图历史）- 使用2D CNN
        self.critic_elevation_encoder = Elevation2DCNNEncoder(
            num_frames=elevation_sampled_frames,
            hidden_dims=cnn_hidden_dims,
            kernel_sizes=cnn_kernel_sizes,
            strides=[2] * len(cnn_hidden_dims),
        ...
        # 1. 高程图VAE编码器：高程图 -> VAE隐变量 (μ, σ) -> z_e - 使用2D CNN
        self.elevation_vae_encoder = Elevation2DCNNEncoder(
            num_frames=elevation_sampled_frames,
            hidden_dims=cnn_hidden_dims,
            kernel_sizes=cnn_kernel_sizes,
            strides=[2] * len(cnn_hidden_dims),
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode12P2_2DCNN 网络结构 (2D CNN版本)")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode12P2_2DCNN_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode12P2_2DCNN策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode12P2_2DCNN_policy.onnx"
        ...
        # 创建ElevationNet Mode12P2_2DCNN专用的导出器
        exporter = _ElevationNetMode12P2_2DCNNOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode12P2_2DCNNOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode12P2_2DCNN策略的ONNX导出器
    
    Mode12P2_2DCNN的推理流程:
    1. 从完整输入中分离：单帧本体观测 + 完整5帧本体历史 + 高程图历史
    2. 高程图编码：高程图 -> elevation_2dcnn_encoder -> elevation_mu -> z_e
    3. 本体编码：5帧历史 -> proprio_encoder -> v_hat, z_p
    4. Actor推理：[单帧本体 + v_hat + z_e + z_p] -> actor -> actions
    """

    def __init__(self, policy: ActorCriticElevationNetMode12P2_2DCNN, normalizer=None, verbose=False):
        super().__init__()
        self.verbose = verbose
        
        # 复制策略所需的模块
        # 高程图VAE编码器和mu head (使用2D CNN)
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - Mode12P2_2DCNN):")
        print(f"{'='*80}")
        ...
        print(f"    2. 高程图编码 (2D CNN) -> z_e ({self.latent_dim}维)")
