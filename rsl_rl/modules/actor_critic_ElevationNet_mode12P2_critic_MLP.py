...
"""
ActorCriticElevationNetMode12P2CriticMLP: VAE架构的编码器-解码器网络

网络结构（修正版）:

    Critic Pipeline (PPO):
        本体观测值（单帧，例如G1中107维）+ 高程图历史值（5帧，例如G1中5×425=2125）
        -> concat后一起进入MLP -> V
...
class ActorCriticElevationNetMode12P2CriticMLP(nn.Module):
    """
    完整的Actor-Critic网络，包含VAE编码器-解码器
    Critic修改：本体观测与高程图一起进入MLP后输出V
    """
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # ============ Critic部分（PPO）- 修改版本 ============
        # 修改：本体观测与高程图一起进入MLP后输出V
        
        # Critic输入维度 = 本体观测 + 高程图（展平）
        # 高程图维度：elevation_sampled_frames * vision_spatial_size[0] * vision_spatial_size[1]
        height, width = vision_spatial_size
        elevation_flat_dim = elevation_sampled_frames * height * width
        critic_input_dim = num_critic_obs + elevation_flat_dim
        
        # Critic网络：[本体观测 + 高程图展平] -> Value
        self.critic = MLP(
            critic_input_dim,
            1,
            critic_hidden_dims,
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode12P2CriticMLP 网络结构")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def evaluate(self, obs: TensorDict, **kwargs: dict[str, Any]) -> torch.Tensor:
        ...
        # 2. 整理高程图格式并展平
        # 高程图形状: [B, 1, T, H, W] 或 [B, T, H, W]
        sampled_height_maps = height_maps.squeeze(1) if height_maps.dim() > 4 else height_maps
        
        # 展平高程图: [B, T, H, W] -> [B, T*H*W]
        elevation_flat = sampled_height_maps.view(sampled_height_maps.shape[0], -1)
        
        # 3. 拼接本体观测和高程图
        # [B, num_critic_obs] + [B, T*H*W] -> [B, num_critic_obs + T*H*W]
        fused_obs = torch.cat((current_proprio_obs, elevation_flat), dim=-1)
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode12P2_critic_MLP_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode12P2CriticMLP策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode12P2_critic_MLP_policy.onnx"
        ...
        # 创建ElevationNet Mode12P2CriticMLP专用的导出器
        exporter = _ElevationNetMode12P2CriticMLPOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode12P2CriticMLPOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode12P2CriticMLP策略的ONNX导出器
    ...
    def __init__(self, policy: ActorCriticElevationNetMode12P2CriticMLP, normalizer=None, verbose=False):
        super().__init__()
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - Mode12P2CriticMLP):")
        print(f"{'='*80}")
