...
"""
ActorCriticElevationNetMode12P2_wo_VAE: 编码器-解码器网络 (无VAE版本)
...
网络结构（修正版）:
    ...
    Encoder-Decoder Pipeline (无VAE架构):
        高程图编码器 (无VAE):
            高程图历史值（critic的高程图，5帧无噪声）-> 2+1D CNN -> 
                -> Linear -> z_t^e (高程图隐向量, 默认32维，直接输出)
...
与Mode12P2的主要区别:
    - 不使用VAE处理高程图 (无VAE, 无重参数化, 无KL散度损失)
    - 高程图编码器直接输出隐向量 z_e
    - 移除 elevation_mu, elevation_logvar
    - 移除 KL损失
    - Actor pipeline中高程图特征直接用于 z_e
"""
...
class ActorCriticElevationNetMode12P2_wo_VAE(nn.Module):
    """
    完整的Actor-Critic网络，包含编码器-解码器 (无VAE版本)
    """
    ...
    def __init__(
        ...
        # 编码器参数（Mode12P2_wo_VAE特有）
        encoder_hidden_dims: list[int] = [256, 256],
        latent_dim: int = 32,  # 高程图隐变量维度
        vel_dim: int = 3,      # 速度维度
        proprio_latent_dim: int = 32,   # 本体隐变量维度
        # 解码器参数（Mode12P2_wo_VAE特有）
        ...
        **kwargs: dict[str, Any],
        ...
        # 1. 高程图编码器：高程图 -> z_e (直接输出，无VAE)
        self.elevation_encoder = R21DElevationEncoder(
            num_frames=elevation_sampled_frames,
            hidden_dims=r21d_hidden_dims,
            kernel_sizes=r21d_kernel_sizes,
            strides=[2] * len(r21d_hidden_dims),
            out_dim=latent_dim,  # 直接输出隐向量维度
            vision_spatial_size=vision_spatial_size,
        )
        
        # 注意：无VAE，所以没有elevation_mu和elevation_logvar
        ...
        # 本体编码器的输出head（输入包含proprio_features + z_e）
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode12P2_wo_VAE 网络结构 (无VAE版本)")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def act(self, obs: TensorDict, **kwargs: dict[str, Any]) -> tuple[torch.Tensor, dict]:
        ...
        # 3. 编码器：获取隐变量（使用无梯度推理）
        with torch.no_grad():
            # 高程图编码 -> z_e (直接输出，无VAE采样)
            elevation_features = self.elevation_encoder(sampled_height_maps)
            z_e = elevation_features  # 直接使用特征向量，无detach需要
            
            # 本体编码 -> v̂, z_p（使用归一化后的完整5帧历史 + z_e）
            proprio_features = self.proprio_encoder(proprio_obs_normalized)
            proprio_z_e_features = torch.cat([proprio_features, z_e], dim=-1)
            v_hat = self.proprio_vel_head(proprio_z_e_features)
            z_p = self.proprio_latent_head(proprio_z_e_features)
    ...
    def act_inference(self, obs: TensorDict) -> tuple[torch.Tensor, dict]:
        ...
        # 高程图编码 -> z_e (直接输出，无VAE)
        elevation_features = self.elevation_encoder(sampled_height_maps)
        z_e = elevation_features  # 直接使用特征向量
        
        # 本体编码（使用归一化后的完整5帧历史 + z_e）
        proprio_features = self.proprio_encoder(proprio_obs_normalized)
        proprio_z_e_features = torch.cat([proprio_features, z_e], dim=-1)
        v_hat = self.proprio_vel_head(proprio_z_e_features)
        z_p = self.proprio_latent_head(proprio_z_e_features)
    ...
    def update_encoder(
        ...
        # 1.2 本体历史编码（使用归一化后的policy观测 + z_e）
        ...
        proprio_z_e_features = torch.cat([proprio_features, z_e.detach()], dim=-1)
        v_hat = self.proprio_vel_head(proprio_z_e_features)
        z_p = self.proprio_latent_head(proprio_z_e_features)
        ...
        # 总损失 (注意：移除了KL损失)
        total_loss = elevation_recon_loss + obs_recon_loss + vel_loss
    ...
    def create_optimizers(self, learning_rate: float) -> dict[str, torch.optim.Optimizer]:
        ...
        # 编码器-解码器优化器 (注意：不包含elevation_mu, elevation_logvar)
        encoder_params = list(self.elevation_encoder.parameters()) + \
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode12P2_wo_VAE_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode12P2_wo_VAE策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode12P2_wo_VAE_policy.onnx"
        ...
        # 创建ElevationNet Mode12P2_wo_VAE专用的导出器
        exporter = _ElevationNetMode12P2_wo_VAEOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode12P2_wo_VAEOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode12P2_wo_VAE策略的ONNX导出器
    
    Mode12P2_wo_VAE的推理流程:
    1. 从完整输入中分离：单帧本体观测 + 完整5帧本体历史 + 高程图历史
    2. 高程图编码：高程图 -> elevation_encoder -> z_e (无VAE，直接输出)
    3. 本体编码：5帧历史 -> proprio_encoder -> v_hat, z_p
    4. Actor推理：[单帧本体 + v_hat + z_e + z_p] -> actor -> actions
    """

    def __init__(self, policy: ActorCriticElevationNetMode12P2_wo_VAE, normalizer=None, verbose=False):
        super().__init__()
        self.verbose = verbose
        
        # 复制策略所需的模块
        # 高程图编码器 (无VAE，直接输出z_e)
        if hasattr(policy, "elevation_encoder"):
            self.elevation_encoder = copy.deepcopy(policy.elevation_encoder)
    ...
    def forward(self, obs_input):
        ...
        # 高程图编码：elevation -> z_e (无VAE，直接输出)
        z_e = self.elevation_encoder(elevation_data)
        
        # 本体编码：5帧历史（归一化后） + z_e -> v_hat, z_p
        # 注意：无VAE，z_e直接使用，不需要detach
        proprio_features = self.proprio_encoder(proprio_history_normalized)
        proprio_z_e_features = torch.cat([proprio_features, z_e], dim=-1)
        v_hat = self.proprio_vel_head(proprio_z_e_features)
        z_p = self.proprio_latent_head(proprio_z_e_features)
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - Mode12P2_wo_VAE):")
        print(f"{'='*80}")
        ...
        print(f"    2. 高程图编码 (R(2+1)D) -> z_e ({self.latent_dim}维) [无VAE]")
