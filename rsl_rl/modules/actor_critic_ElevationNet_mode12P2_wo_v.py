...
"""
ActorCriticElevationNetMode12P2_wo_v: VAE架构的编码器-解码器网络 (无线速度估计版本)
...
网络结构（修正版）:
    ...
    Encoder-Decoder Pipeline (VAE架构):
        ...
        本体解码器:
            [z_t^e + z_t^p] -> MLP -> 重建下一帧本体信息（num_decode维）
    
    Actor Pipeline:
        [单帧本体观测值（例如G1中96维）+ z_t^e + z_t^p] -> MLP Actor -> a_t
...
与Mode12P2的主要区别:
    - 不估计线速度 (移除 v̂_t)
    - Actor输入: [单帧本体 + z_e + z_p] (不包含v_hat)
    - 解码器输入: [z_e + z_p] (不包含v_hat)
    - 移除vel_loss
"""
...
class ActorCriticElevationNetMode12P2_wo_v(nn.Module):
    """
    完整的Actor-Critic网络，包含VAE编码器-解码器 (无线速度估计版本)
    """
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # 4. 本体解码器：[z_e + z_p] -> 重建下一帧本体信息 (不包含v_hat)
        decoder_input_dim = latent_dim + proprio_latent_dim
        ...
        # Actor网络：[单帧本体观测 + z_e + z_p] -> 动作 (不包含v_hat)
        # 单帧本体观测维度：num_proprio_one_frame（例如G1中为96）
        # z_e维度：latent_dim（高程图VAE隐变量，默认32）
        # z_p维度：proprio_latent_dim（本体隐变量，默认32）
        actor_input_dim = self.num_proprio_one_frame + latent_dim + proprio_latent_dim
        self.actor = MLP(
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode12P2_wo_v 网络结构 (无线速度估计)")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def act(self, obs: TensorDict, **kwargs: dict[str, Any]) -> tuple[torch.Tensor, dict]:
        ...
        # 4. 融合特征 (不包含v_hat)
        fused_features = torch.cat([current_proprio_obs, mu_e, z_p], dim=-1)
    ...
    def update_encoder(
        ...
        # 总损失 (注意：移除了vel_loss)
        total_loss = elevation_recon_loss + obs_recon_loss + self.kl_weight * kl_loss
    ...
    def create_optimizers(self, learning_rate: float) -> dict[str, torch.optim.Optimizer]:
        ...
        # 编码器-解码器优化器 (注意：不包含proprio_vel_head)
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode12P2_wo_v_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode12P2_wo_v策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode12P2_wo_v_policy.onnx"
        ...
        # 创建ElevationNet Mode12P2_wo_v专用的导出器
        exporter = _ElevationNetMode12P2_wo_vOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode12P2_wo_vOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode12P2_wo_v策略的ONNX导出器
    
    Mode12P2_wo_v的推理流程:
    1. 从完整输入中分离：单帧本体观测 + 完整5帧本体历史 + 高程图历史
    2. 高程图编码：高程图 -> elevation_vae_encoder -> elevation_mu -> z_e
    3. 本体编码：5帧历史 -> proprio_encoder -> z_p (不估计速度)
    4. Actor推理：[单帧本体 + z_e + z_p] -> actor -> actions
    """

    def __init__(self, policy: ActorCriticElevationNetMode12P2_wo_v, normalizer=None, verbose=False):
        super().__init__()
    ...
    def forward(self, obs_input):
        ...
        # 融合特征 (不包含v_hat)
        fused_features = torch.cat([current_proprio_obs, z_e, z_p], dim=-1)
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - Mode12P2_wo_v):")
        print(f"{'='*80}")
        ...
        print(f"    3. 5帧历史编码 -> z_p ({self.proprio_latent_dim}维) [不估计速度]")
        print(f"    4. Actor: [单帧 + z_e + z_p] -> actions")
        print(f"{'='*80}\n")
