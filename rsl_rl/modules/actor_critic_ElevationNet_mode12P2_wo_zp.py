...
"""
ActorCriticElevationNetMode12P2_wo_zp: VAE架构的编码器-解码器网络 (不估计下一时刻观测值版本)
...
网络结构（修正版）:
    ...
    Actor Pipeline:
        [单帧本体观测值（例如G1中96维）+ v̂_t + z_t^e] -> MLP Actor -> a_t
...
与Mode12P2的主要区别:
    - 不估计下一时刻的观测值 (移除 obs_decoder 和 obs_recon_loss)
    - 保留速度估计 v̂_t
    - 保留 z_e，移除 z_p
    - 只保留高程图重建损失和KL散度损失
"""
...
class ActorCriticElevationNetMode12P2_wo_zp(nn.Module):
    """
    完整的Actor-Critic网络，包含VAE编码器-解码器 (不估计下一时刻观测值版本)
    """
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # 注意：移除了obs_decoder，因为不估计下一时刻的观测值
        
        # ============ Actor部分 ============
        
        # Actor网络：[单帧本体观测 + v̂ + z_e] -> 动作
        # 单帧本体观测维度：num_proprio_one_frame（例如G1中为96）
        # v̂维度：vel_dim（速度估计，通常为3）
        # z_e维度：latent_dim（高程图VAE隐变量，默认32）
        actor_input_dim = self.num_proprio_one_frame + vel_dim + latent_dim
        self.actor = MLP(
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode12P2_wo_zp 网络结构 (不估计下一时刻观测值)")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def act(self, obs: TensorDict, **kwargs: dict[str, Any]) -> tuple[torch.Tensor, dict]:
        ...
        # 4. 融合特征
        fused_features = torch.cat([current_proprio_obs, v_hat, mu_e], dim=-1)
    ...
    def update_encoder(
        ...
        Args:
            obs_batch: 当前观测batch
            next_obs_batch: 下一时刻观测batch (用于速度估计)
        ...
        # 3.2 KL散度（VAE的正则化项）- 使用per-dimension平均而不是sum
        # 这样KL损失的scale与latent_dim无关
        kl_loss = -0.5 * torch.mean(1 + logvar_e - mu_e.pow(2) - logvar_e.exp())
        
        # 3.3 速度估计损失
        vel_target = next_obs_batch["critic"][:, -3:]  
        vel_loss = F.mse_loss(v_hat, vel_target)
        
        # 总损失 (注意：移除了obs_recon_loss)
        total_loss = elevation_recon_loss + self.kl_weight * kl_loss + vel_loss
    ...
    def create_optimizers(self, learning_rate: float) -> dict[str, torch.optim.Optimizer]:
        ...
        # 编码器-解码器优化器 (注意：不包含obs_decoder和proprio_latent_head)
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode12P2_wo_zp_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode12P2_wo_zp策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode12P2_wo_zp_policy.onnx"
        ...
        # 创建ElevationNet Mode12P2_wo_zp专用的导出器
        exporter = _ElevationNetMode12P2_wo_zpOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode12P2_wo_zpOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode12P2_wo_zp策略的ONNX导出器
    
    Mode12P2_wo_zp的推理流程:
    1. 从完整输入中分离：单帧本体观测 + 完整5帧本体历史 + 高程图历史
    2. 高程图编码：高程图 -> elevation_vae_encoder -> elevation_mu -> z_e
    3. 本体编码：5帧历史 -> proprio_encoder -> v_hat
    4. Actor推理：[单帧本体 + v_hat + z_e] -> actor -> actions
    """

    def __init__(self, policy: ActorCriticElevationNetMode12P2_wo_zp, normalizer=None, verbose=False):
        super().__init__()
    ...
    def forward(self, obs_input):
        ...
        # 融合特征
        fused_features = torch.cat([current_proprio_obs, v_hat, z_e], dim=-1)
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - Mode12P2_wo_zp):")
        print(f"{'='*80}")
        ...
        print(f"    3. 5帧历史编码 -> v_hat ({self.vel_dim}维)")
        print(f"    4. Actor: [单帧 + v_hat + z_e] -> actions")
        print(f"  ")
        print(f"  注意: 不估计下一时刻的观测值 (移除了obs_decoder)")
        print(f"{'='*80}\n")
