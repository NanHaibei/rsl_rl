...
"""
ActorCriticElevationNetMode7: VAE编码器架构，使用R(2+1)D CNN处理高程图序列 + Transformer融合网络

网络结构:
    本体 -> 本体编码器MLP -> 特征1
    高程图序列 -> R(2+1)D CNN编码器(空间2D卷积 + 时间1D卷积) -> 特征2
    [特征1 + 特征2] -> Transformer融合网络 -> 编码特征
...
class TransformerFusionEncoder(nn.Module):
    """Transformer融合编码器，用于融合本体特征和视觉特征"""
    ...
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=hidden_dim,
            nhead=num_heads,
            dim_feedforward=hidden_dim * 4,  # FFN隐藏层维度通常是4倍的d_model
            dropout=dropout,
            activation='gelu',  # Transformer通常使用GELU激活函数
            batch_first=True,
...
class ActorCriticElevationNetMode7(nn.Module):
    """Mode7: 使用R(2+1)D CNN处理高程图序列（空间2D卷积 + 时间1D卷积）+ Transformer融合网络 + VAE架构
    
    网络组成:
    1. 本体编码器MLP - 提取本体特征
    2. 高程图R(2+1)D CNN编码器 - 分解的时空卷积提取特征
    3. Transformer融合网络 - 融合特征（替代MLP）
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        ########################################## Actor ##############################################
        print("\n" + "=" * 80)
        print("🌟 网络架构: ElevationNet Mode7 (R(2+1)D CNN + Transformer + VAE)")
        print("=" * 80)
        print("✓ Mode7: 本体编码器 + 高程图R(2+1)D CNN编码器(空间2D+时间1D) + Transformer融合网络 + VAE -> 动作")
        ...
        print(f"  3. Transformer融合网络: {fusion_input_dim} (本体{proprio_feature_dim} + 视觉{vision_feature_dim}) -> {fusion_output_dim}")
        print(f"     (hidden_dim={transformer_hidden_dim}, heads={transformer_num_heads}, layers={transformer_num_layers})")
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode7_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode7策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode7_policy.onnx"
        ...
        # 创建ElevationNet Mode7专用的导出器
        exporter = _ElevationNetMode7OnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode7OnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode7策略的ONNX导出器"""

    def __init__(self, policy: ActorCriticElevationNetMode7, normalizer=None, verbose=False):
        super().__init__()
