...
"""
ActorCriticElevationNetMode8: VAE编码器架构，使用R(2+1)D CNN处理高程图序列 + MLP融合网络

网络结构:
    历史本体观测 -> 直接进入融合层（不再经过中间MLP）
    高程图序列 -> R(2+1)D CNN编码器(空间2D卷积 + 时间1D卷积) -> 视觉特征
    [历史本体观测 + 视觉特征] -> MLP融合网络 -> 编码特征
...
class MLPFusionEncoder(nn.Module):
    """MLP融合编码器，用于融合历史本体观测和视觉特征"""
    ...
    def __init__(
        ...
        for hidden_dim in hidden_dims:
            ...
            layers.append(nn.ELU() if activation == "elu" else nn.ReLU())
            if dropout > 0:
...
class ActorCriticElevationNetMode8(nn.Module):
    """Mode8: 使用R(2+1)D CNN处理高程图序列（空间2D卷积 + 时间1D卷积）+ MLP融合网络 + VAE架构
    
    网络组成:
    1. 历史本体观测 -> 直接进入融合层（不经过MLP）
    2. 高程图R(2+1)D CNN编码器 - 分解的时空卷积提取特征
    3. MLP融合网络 - 融合历史本体观测和视觉特征
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        ########################################## Actor ##############################################
        print("\n" + "=" * 80)
        print("🌟 网络架构: ElevationNet Mode8 (R(2+1)D CNN + MLP Fusion + VAE)")
        print("=" * 80)
        print("✓ Mode8: 历史本体观测直接进入融合 + 高程图R(2+1)D CNN编码器(空间2D+时间1D) + MLP融合网络 + VAE -> 动作")
        
        # 注意：Mode8中不再有本体编码器MLP，历史本体观测直接进入融合层
        print(f"  1. 历史本体观测: 直接进入融合层 (不经过MLP)")
        ...
        # 3. MLP融合网络（替代Transformer）
        fusion_output_dim = encoder_hidden_dims[-1]
        # 融合网络输入：历史本体观测维度 + 视觉特征维度
        # 历史本体观测维度：num_actor_obs (历史帧数的本体观测总维度)
        fusion_input_dim = num_actor_obs + vision_feature_dim
        self.fusion_encoder = MLPFusionEncoder(
        ...
        print(f"  3. MLP融合网络: {fusion_input_dim} (历史本体{num_actor_obs} + 视觉{vision_feature_dim}) -> {fusion_output_dim}")
        print(f"     (hidden_dims={fusion_hidden_dims}, dropout={fusion_dropout})")
    ...
    def encoder_forward(self, proprio_obs: torch.Tensor, obs: TensorDict):
        """编码器前向传播"""
        # 1. 历史本体观测直接进入融合层（不经过MLP）
        # proprio_obs 已经是历史帧数的本体观测，直接使用
    ...
    def create_optimizers(self, learning_rate: float) -> dict[str, torch.optim.Optimizer]:
        ...
        encoder_optimizer = optim.Adam([
            # 注意：Mode8中没有本体编码器，所以不包含self.proprio_encoder
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode8_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode8策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode8_policy.onnx"
        ...
        # 创建ElevationNet Mode8专用的导出器
        exporter = _ElevationNetMode8OnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode8OnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode8策略的ONNX导出器"""

    def __init__(self, policy: ActorCriticElevationNetMode8, normalizer=None, verbose=False):
        super().__init__()
