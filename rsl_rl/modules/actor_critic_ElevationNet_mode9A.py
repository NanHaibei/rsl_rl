...
"""
ActorCriticElevationNetMode9A: R(2+1)D CNN架构，分解时空卷积处理高程图序列

网络结构:
    高程图历史(50帧) -> 采样(隔10帧抽1帧) -> 5帧序列 -> R(2+1)D CNN -> 64维特征向量
    本体观测(当前帧131维) -> 直接使用
...
class ActorCriticElevationNetMode9A(nn.Module):
    """Mode9A: R(2+1)D CNN架构，分解时空卷积处理高程图序列
    
    网络组成:
    1. 高程图采样器: 从50帧历史中隔10帧抽取5帧
    2. R(2+1)D CNN编码器: 分解的时空卷积，输出64维特征
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        self.elevation_encoder_actor = R2Plus1DElevationEncoder(
            ...
            strides=[2] * len(r2p1d_hidden_dims),  # 默认使用stride=2
        ...
        # 打印网络结构
        print("\n" + "="*80)
        print("ActorCriticElevationNetMode9A 网络结构 (R(2+1)D CNN)")
        print("="*80)
        print(self)
        print("="*80 + "\n")
    ...
    def export_to_onnx(self, path: str, filename: str = "ElevationNet_mode9A_policy.onnx", normalizer: torch.nn.Module | None = None, verbose: bool = False) -> None:
        """将ElevationNet Mode9A策略导出为ONNX格式
        
        Args:
            path: 保存目录的路径
            filename: 导出的ONNX文件名，默认为"ElevationNet_mode9A_policy.onnx"
        ...
        # 创建ElevationNet Mode9A专用的导出器
        exporter = _ElevationNetMode9AOnnxPolicyExporter(self, normalizer, verbose)
        exporter.export(path, filename)


class _ElevationNetMode9AOnnxPolicyExporter(torch.nn.Module):
    """ElevationNet Mode9A策略的ONNX导出器"""

    def __init__(self, policy: ActorCriticElevationNetMode9A, normalizer=None, verbose=False):
        super().__init__()
    ...
    def export(self, path, filename):
        ...
        print(f"\n{'='*80}")
        print(f"ONNX导出配置 (单输入模式 - R(2+1)D CNN):")
        print(f"{'='*80}")
