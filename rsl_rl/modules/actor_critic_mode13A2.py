...
"""
ActorCriticMode13A2: 2DCNN处理单帧高程图 + 本体观测直接融合
...
网络结构:
    ...
    Actor Pipeline:
    - 单帧高程图 -> 2DCNN提取特征
    - 本体观测(原始) + 视觉特征 -> concat -> Actor网络 -> Actions
"""
...
class ActorCriticMode13A2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # Actor输入维度 = 本体观测 + 视觉特征
        actor_input_dim = num_actor_obs + vision_feature_dim
        ...
        print(f"  Actor input dim: {actor_input_dim} (proprio: {num_actor_obs} + vision feature: {vision_feature_dim})")
...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy: ActorCriticMode13A2, normalizer=None, verbose=False):
        super().__init__()
        ...
        # 从actor获取本体观测维度（actor输入维度减去视觉特征维度）
        self.vision_feature_dim = policy.vision_feature_dim
        self.proprio_obs_dim = policy.actor[0].in_features - self.vision_feature_dim
